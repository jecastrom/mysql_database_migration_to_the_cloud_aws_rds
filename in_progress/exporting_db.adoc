...
First a global read lock is set by the global Shell session using the  `FLUSH TABLES WITH READ LOCK` statement.

[NOTE]
====
`FLUSH TABLES WITH READ LOCK` closes all open tables and locks all tables for all databases with global read lock.
====
