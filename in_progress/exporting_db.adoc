...
:idseparator: -
:linkattrs:
:banner: https://user-images.githubusercontent.com/63274055/160254383-fba6d3c3-6f83-4c1f-8eb0-974bc1bd4e2f.png
...
To create the dump I used the MySQL Shell Schema Dump Instance
 ...
 * Depending on por use case, If the `binary log` is required to be included in the dump, the user running the utility needs `REPLICATION CLIENT` privileges. Otherwise the dump will continue as normal but without the `binary log`. A warning message is displayed:
...
```
util.dumpInstance("C:/Users/jcast/data/my_instance_dump"")
```
====
''''
====
