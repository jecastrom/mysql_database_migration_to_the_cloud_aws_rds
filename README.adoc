...
To obtain the endpoint of the RDS DB, this is possible with just calling a `describe-db-instances statement`. Now to only retrieve the Endpoint Address, we can use the `--filter` or `--query` options to filter responses. As the --filter option is supported by a limited number of AWS commands and sub-commands, I prefer to use `--query`. This option can be used with all AWS commands and uses exclusively the `JMESPath` JSON scripting language.
...
In this way we are getting the information we want. However with the help of the `db-instance-available` command, we can combine the three commands and ask AWS: 
