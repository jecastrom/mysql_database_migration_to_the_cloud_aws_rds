...
* AWS account
* AWS CLI v2 + JMESPath query language
...
Once we are done with the migration, we can de-associate and parameter from the RDS instance.
...
```
 aws rds describe-db-instances wait db-instance-available --db-instance-identifier sakila-aws --query 'DBInstances[].Endpoint[].Address[]' --output json
```


The output tells us the status of the RDS instance creation is "Creating". Therefore the endpoint address is not available yet.
